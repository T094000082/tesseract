...
echo FileName,Text,Timestamp > "%CSV_FILE%"
...
for %%f in ("%INPUT_FOLDER%\*.jpg" "%INPUT_FOLDER%\*.jpeg" "%INPUT_FOLDER%\*.png" "%INPUT_FOLDER%\*.bmp" "%INPUT_FOLDER%\*.tiff" "%INPUT_FOLDER%\*.gif") do (
    if exist "%%f" (
        ...
        "%TESSERACT_PATH%" "%%f" temp_ocr_result -l eng+chi_tra --psm 6 2>nul
        
        if exist temp_ocr_result.txt (
            echo   OCR 成功，正在寫入 CSV...
            REM 直接用 type 和 echo 處理，避免複雜的 PowerShell 命令
            for /f "usebackq delims=" %%a in ("temp_ocr_result.txt") do (
                echo "%%~nxf","%%a","%date% %time%" >> "%CSV_FILE%"
            )
            del temp_ocr_result.txt 2>nul
            echo   ✓ 完成
        ) else (
            echo "%%~nxf","[OCR 處理失敗]","%date% %time%" >> "%CSV_FILE%"
            echo   ✗ 失敗
        )
        echo.
    )
...
echo.
echo 注意: 如果中文顯示為亂碼，請用以下方式開啟：
echo 1. 用 VS Code 開啟 CSV 檔案
echo 2. 或在 Excel 中：資料 ^> 取得資料 ^> 從文字/CSV ^> 選擇 UTF-8 編碼
echo.
pause
